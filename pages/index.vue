      ...
      <div class="container">
        ...
        <div class="about-grid">
          ...
          <div class="about-content" v-reveal.slide-right>
            ...
            <p class="about-paragraph">
              凭借深厚的技术积累与行业洞察，我们打造了涵盖叉车残值智能评估、叉车维修培训认证、二手叉车信息撮合交易、AI叉车智能助手等核心服务体系，帮助客户实现降本增效、科学决策，推动叉车行业迈向智能化新时代。
            </p>
            <div class="stat-grid">
              <div class="stat-card">
      ...
      <div class="container narrow">
        ...
        <div class="founder-quote-wrap" v-reveal="300">
          ...
          <blockquote class="founder-quote">
            华南理工大学教授、博导，国家及省市级项目评审咨询专家，曾挂职江门市工信局副局长。深耕工业互联网与产业数字化转型领域，发表学术论文八十余篇，获专利及软著六十余项，主持国家级等各类项目二十余项。牵头研发多款智能监测系统与产业互联网平台并落地应用，创办多家企业并任董事长，带领硕博团队深耕产学研融合与科研成果产业化。
          </blockquote>
        </div>
      </div>
    </section>
      ...
      >
        ...
        <div ref="featuredTrackRef" class="featured-track" @scroll.passive="onFeaturedScroll">
          <article
            ...
            :key="`${idx === featuredList.length ? 'clone' : 'real'}-${item.content_id}`"
            class="featured-card"
            :aria-hidden="idx === featuredList.length ? 'true' : undefined"
          >
        ...
        <!-- 分页指示器 -->
        <div class="featured-dots" v-if="featuredList.length > 1">
          <button
...
const { subdomain, site } = useSiteLinks()
...
const featuredList = ref<FeaturedListItem[]>([])
...
const displayList = computed<FeaturedListItem[]>(() => {
  const list = featuredList.value
  if (list.length <= 1) return list
  return [...list, list[0]!]
})

// 精选内容：预渲染阶段服务端取数（构建时嵌入静态 HTML），客户端 hydration 复用载荷不重取。
...
// 之前仅在 onMounted 填充会导致首屏 HTML 轮播为空。
featuredList.value = ((featuredData.value?.items as unknown) || []) as FeaturedListItem[]
...
function scrollRightWithLoop() {
  const totalCount = featuredList.value.length
...
function featuredAutoplayNext() {
  ...
  const track = featuredTrackRef.value
  if (!track || featuredList.value.length === 0) return
  scrollRightWithLoop()
}
